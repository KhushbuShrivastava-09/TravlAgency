...
const UpcomingTrips = () => {
  const mockTrips = [
    { id: 1, title: "Spiti Full Circuit Roadtrip", image: "/images/spiti-roadtrip.jpg", duration: "N/7D", location: "Delhi - Delhi", dates: "9 Aug, 5 Sep + 4 batches", price: "₹22,999 onwards" },
    { id: 2, title: "8-Day Leh Ladakh Tour Package with Turtuk", image: "/images/leh-ladakh.jpg", duration: "7N/8D", location: "Leh - Leh", dates: "30 Aug, 6 Sep + 4 batches", price: "₹28,999 onwards" },
    { id: 3, title: "Ladakh Bike Trip from Srinagar to Leh", image: "/images/ladakh-bike.jpg", duration: "0N/11D", location: "Srinagar - Delhi", dates: "0 Aug", price: "₹42,999 onwards" },
    { id: 4, title: "15 Days Summer Special European Grandeur", image: "/images/europe-grandeur.jpg", duration: "14N/15D", location: "London Airport - Rome Airport", dates: "30 Aug, 9 Sep + 4 batches", price: "₹2,90,900 onwards" },
    { id: 5, title: "13 Days Imperial Europe Trip with London", image: "/images/imperial-europe.jpg", duration: "12N/13D", location: "London Airport - Rome Airport", dates: "30 Aug, 3 Sep + 4 batches", price: "₹2,92,900 onwards" },
  ];

  const mockDestinations = {
    india: ["Ladakh", "Spiti", "Meghalaya", "Kashmir", "Kerala", "Andaman", "Rajasthan", "Himachal", "Sikkim", "Uttarakhand"],
    international: ["Europe", "America", "Asia", "Australia"],
  };

  const mockMonths = ["Aug-25", "Sep-25", "Oct-25", "Nov-25", "Dec-25", "Jan-26", "Feb-26", "Mar-26", "Apr-26", "May-26", "Jun-26"];
  ...
  const [sliderChanged, setSliderChanged] = useState({ duration: false, budget: false });
  ...
  const applyFilters = () => {
    let filteredTrips = mockTrips.filter((trip) => {
      const durationMatch = trip.duration.match(/(\d+)N/)?.[1]
        ...
        : true;
      const budgetMatch = parseInt(trip.price.replace(/[^0-9]/g, "")) >= filters.budget[0] &&
        parseInt(trip.price.replace(/[^0-9]/g, "")) <= filters.budget[1];
      const subDestMatch = filters.subDestinations.length === 0 || filters.subDestinations.some((sub) => trip.title.includes(sub));
      const monthMatch = filters.months.length === 0 || filters.months.some((month) => trip.dates.includes(month.split("-")[0]));
  ...
  // Dynamic style for range slider track based on sliderChanged state
  ...
  return (
    <div className={styles.container}>
        ...
        <aside className={styles.filters}>
          ...
          <div className={styles.filterSection}>
            ...
            <div className={styles.destinationButtons}>
              <button
                className={`${styles.destButton} ${filters.destination === "India" ? styles.activeDest : ""}`}
                onClick={() => handleDestinationClick("India")}
              >
                India
              </button>
              <button
                className={`${styles.destButton} ${filters.destination === "International" ? styles.activeDest : ""}`}
          ...
          <div className={styles.filterSection}>
            ...
            <section className={styles.rangeSlider} style={getRangeTrackStyle("duration")}>
              <input
          ...
          <div className={styles.filterSection}>
            ...
            <section className={styles.rangeSlider} style={getRangeTrackStyle("budget")}>
              <input
          ...
          <div className={styles.monthCarousel}>
            <div className={styles.carouselInner}>
              {months.map((month) => (
                <button
                  key={month}
                  className={`${styles.monthButton} ${filters.months.includes(month) ? styles.activeMonth : ""}`}
                  onClick={() => handleMonthChange({ target: { value: month } })}
                >
                  {month.split("-")[0]}
                </button>
              ))}
            ...
            {trips.map((trip, index) => (
              ...
              >
                <div className={styles.tripImageContainer}>
                  <Image src={trip.image} alt={trip.title} width={300} height={200} className={styles.tripImage} />
                </div>
                <div className={styles.tripInfo}>
                  ...
                  <p className={styles.price}><strong>Price:</strong> {trip.price}</p>
      ...
      {showSubDestinations && (
        <div className={styles.subDestinationsOverlay}>
          <div className={styles.subDestinationsPanel}>
            <div className={styles.closeIcon} onClick={closeSubDestinations}>
              <FaTimes />
            </div>
            <div className={styles.subDestinations}>
              {(filters.destination === "India" ? destinations.india : destinations.international).map((dest) => (
